...
public class RestaurantServiceImpl implements IRestaurantService{
	...
	public Map<String,List<RestaurantBase>> getRestaurantByLocation(RestauantFindRequestType request) {
		...
		if(null!=request.getLocationIds() && !request.getLocationIds().isEmpty()){
			...
			restaurantMetaDataByType.put(Constants.RESTAURANT_BY_NO_AREA_SELECTED, restaurantsByCityId);
		}
	...
	public Map<String, List<RestaurantBase>> formatRestaruantData(Map<String, List<RestaurantBase>> restaurantData) {
		restaurantData.forEach((k,v)->{
			v.forEach(restaurant->{
				restaurant.setArea(staticDataService.getAreaById(restaurant.getCityId(),
						restaurant.getAreaId()).getName());
				restaurant.setCity(staticDataService.getCityById(restaurant.getCityId()).getName());
				restaurant.setLocation(staticDataService.getLocation(restaurant.getCityId(), 
						restaurant.getAreaId(), restaurant.getLocationId()).getName());
	...
	@Override
	public RestaurantMetaDataRepsoneType collateRestaurantData(Map<String, List<RestaurantBase>> restaurantData) {
